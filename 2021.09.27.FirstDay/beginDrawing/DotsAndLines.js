...
function drawScreen(gl, n) {
  ...
  for (var i = 0; i < n; ++i) {
    ...
    var radius_jitter = Math.random() * max_jitter - (max_jitter / 2);
    var radius = r + radius_jitter;
    verts[8*i    ] = radius * Math.cos(curr_angle) / aspect_ratio;
    verts[8*i + 1] = radius * Math.sin(curr_angle);
    verts[8*i + 2] = 0.0;
    verts[8*i + 3] = 1.0;
    verts[8*i + 4] = rgb.r;
    verts[8*i + 5] = rgb.g;
...
// first, create an array with all our vertex attribute values:
  ...
  var vertices = new Float32Array(8 * n);
  ...
  for (var i = 0; i < n; ++i) {
    var radius_jitter = Math.random() * max_jitter - (max_jitter / 2);
    var radius = r + radius_jitter;
    vertices[8*i    ] = radius * Math.cos(curr_angle);
    vertices[8*i + 1] = radius * Math.sin(curr_angle);
    vertices[8*i + 2] = 0.0;
    vertices[8*i + 3] = 1.0;
    // Intermediary values will be set in the main loop
    vertices[8*i + 7] = 1.0;
    curr_angle += angle;
  }
